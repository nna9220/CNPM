...
@WebServlet(urlPatterns = {"/taohoidong5"})
public class InsertGiangVien5Controller extends HttpServlet {
		...
		try {
			...
			for (FileItem item : items) {
				...
				if (item.getFieldName().equals("ma_giangvien")) {
					hoidong.setGiangvien5(item.getString("UTF-8"));
